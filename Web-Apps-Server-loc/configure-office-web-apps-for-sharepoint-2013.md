...
Avant d’entamer les procédures suivantes, assurez-vous d’avoir configuré Office Web Apps Server en effectuant les étapes décrites dans la section [Déploiement d'une batterie Office Web Apps Server à serveur unique qui utilise le protocole HTTPS](deploy-office-web-apps-server.md) ou [Déployer une batterie Office Web Apps Server multiserveur à charge équilibrée qui utilise le protocole HTTPS](deploy-office-web-apps-server.md).
